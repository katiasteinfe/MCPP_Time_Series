...
def compute_dwell_metrics_adults(I2MW_fix_clean, output_dir):
    '''
    ...
    for (pid, trial), df in I2MW_fix.groupby(['Participant','Trial'], sort=False):
        ...
        # as a record
        trial_total_end = df['trial_end_time'].iloc[0]-df['trial_start_time'].iloc[0]
        ...
        age = 11 #set to 11 for adults for future plotting
    
        if not age:
            age = 0
            
        group = 0 # group =0 for adults df.Group.iloc[0]
        
        fix_count_total = df.iloc[-1].fix_count
        
        fix_count_faces = df.loc[df.fAOI != '-'].fix_count.count()
        
        row = {'Participant': pid, 'Trial': trial, 'Condition': cond, 'Actor':actor, 'Iteration':iteration,'Age':age, 'Target':targ, 'Stimulus':stim, 'Group':group,
        ...
        p_FA =counts / faces_total_arr if (counts.sum()> 0) & (faces_total_arr > 0) else np.zeros_like(counts, dtype=float)
        ...
        # find the first fixation on the target 
        mask = (df['fAOI'] == targ)
        
        if (mask==True).any():
            first_time = df[1:].loc[mask, 'fix_start'].iloc[0]  #only the 2nd fixation of the trial onwards is considered
            row['target_found']=1
            row['latency']=first_time
            
            if first_time > trial_total:
                print(f'!! error, check {pid}, {trial} - lat > trial end')
        else: # no fixations on the target in this trial
    ...
    # Second-pass through async trial
    for (pid, target, actor, iteration), sync_trial  in I2MW_fix.loc[I2MW_fix.Condition=='Sync'].groupby(['Participant','Target','Actor','Iteration'], sort=False):
        ...
        age = 11#async_trial.Age.iloc[0]
        group = 0#async_trial.Group.iloc[0]
        if pd.isna(age):
            age = 0
        ...
        # as a record
        trial_total_end = async_trial['trial_end_time'].iloc[0]-async_trial['trial_start_time'].iloc[0]
        ...
        # establish probabilities of fixation on each face / total face fixations
        counts = face_sums.reindex(face_labels, fill_value=0).to_numpy(dtype=float)
        faces_total_arr = np.asarray(  [faces_total], dtype=float)
        p_FA =counts / faces_total_arr if (counts.sum() != 0) & (faces_total_arr > 0) else np.zeros_like(counts, dtype=float)
        # entropy on face time / total faces time
        ...
        # establish probabilities of fixation on each distractor / total distractor fixations
        counts_distractors = face_sums.loc[face_sums.index != targ].to_numpy(dtype=float)
        counts_distractors=counts_distractors[counts_distractors>0]
        total_FA_dist_time = faces_total_arr-face_sums.loc[face_sums.index == targ].sum()
        p_dist_FA =counts_distractors / total_FA_dist_time if (counts.sum()> 0) & (total_FA_dist_time > 0) else np.zeros_like(counts_distractors, dtype=float)
        # entropy on distractor time / total distractors time
        row['STE_distractors_PFA'] = shannon_entropy_bits(p_dist_FA)
        row['normSTE_distractors_PFA'] = normalized_entropy(p_dist_FA, n_states=len(face_labels)-1)
    ...
    DwellMetrics.to_pickle(os.path.join(output_dir, 'DwellMetrics_Adults.pkl'))
